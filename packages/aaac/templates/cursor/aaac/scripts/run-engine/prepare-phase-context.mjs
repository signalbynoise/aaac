  ...
  const context = {
    ...
    instructions:
      experience?.reuse_mode === "delta_or_confirm"
        ...
        : "Read this file only — do not load full prior swarm transcripts. Return structured blocks per agent spec. Honor experience.strategy and experience.execution (prioritize/skip). Prefer experience.repo_memory: read focus_spans envelopes first (path:envelope_start-envelope_end), then widen to full symbol span, then full file only for gaps; verify focus_paths/invariants; treat impact, entry_flows, clusters, and call_neighbors as verified structure (do not re-walk callers/callees already listed). Prefer experience.repo_facts over rediscovery. Cite experience.lessons with evidence when following recommendations. Stay within experience.context_budget.",
  };
