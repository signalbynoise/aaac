...
- **SSOT:** do not invent constants, routes, or file paths. **Finding is graph-native** — use the inlined repo packet (`focus_paths`, `read_pack`). Read only granted paths. If you need more SOURCE context, call `request_context`. Do not Glob/Grep/list directories or retry illegal Reads.
