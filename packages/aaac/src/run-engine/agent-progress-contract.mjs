...
function resolveCompletionMetrics(options) {
  return {
    tokens: options.tokens ?? parseDetailMetric(options.detail, "tokens"),
...
function buildCompletedAgentEntry(prior, completion) {
  const { phase, phaseSlot, completedAt, options, tokens, context } = completion;
  ...
  const entry = {
    ...
    ...completedTokenFields(prior, options, tokens, context),
